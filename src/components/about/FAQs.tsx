  ...
  return (
    <section className="bg-white py-28">
      <div className="max-w-6xl mx-auto px-4 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 bg-[#ffffff] rounded-xl overflow-hidden">
          {/* Left: Questions */}
          <div className="bg-white divide-y divide-gray-200 border-[1px] border-[#E5F5FC] rounded-xl">
            {questions.map((question, index) => (
              <button
                key={index}
                className={`w-full px-6 py-4 text-left flex items-center justify-between transition ${
                  activeIndex === index
                    ? "bg-[#E5F5FC] font-semibold text-[#005BA9]"
                    : "hover:bg-[#F0F8FF] text-gray-800"
                }`}
              ...
              >
                <span className="flex items-center gap-3">
                  <div
                    className={`w-3 h-3 rounded-full ${
                      activeIndex === index ? "bg-[#0071CE]" : "bg-[#B8DBF0]"
                    }`}
                  ></div>
                  {question}
                </span>
                <svg
                  className={`w-4 h-4 transform transition-transform duration-200 ${
                    activeIndex === index ? "-rotate-90 text-[#005BA9]" : ""
                  }`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
          ...
          {/* Right: Answers */}
          <div className="bg-[#F4FAFF] p-8 rounded-xl text-gray-800 text-base space-y-6">
            <h4 className="font-semibold text-[#005BA9] mb-2">
